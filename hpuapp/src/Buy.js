...
const Buy = ()=>{
  ...
  const handleSubmit =(e)=>{
    ...
    const fetchData= async()=>{
      setLoading(true);
      try{
      let result = await fetch(`http://127.0.0.1:5000/locationbuy?place=${property}`);
    ...
    return(
      ...
      <h1 style={{ marginTop:"2%",color:"skyblue", textShadow:"2px 4px 6px purple"}}>Search your propety here</h1>
 <form onSubmit={handleSubmit} class="d-flex" role="search" style={{margin:"100px", paddingLeft:"300px", paddingRight:"300px"}}>
     ...
     { loading ? "loading......." 
      : <div>{ data.map(items=>(<Buylist items={items} keys={items._id}/>))}</div>
      }
      </div>
